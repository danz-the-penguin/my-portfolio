// public/solverWorker.js - Zero-GC Binary GADDAG + Full Equity + Minimax Engine
...
function evaluateLeaveEquity(counts, blanksRemaining) {
  ...
  for (let c = 0; c < 26; c++) {
    ...
    if (c === 0 || c === 4 || c === 8 || c === 14 || c === 20) vowels += count;
    else consonants += count;
...
function findOpponentBestScore(
  ...
  const oppCrossV = new Uint32Array(225);
  const oppCrossScoreV = new Int16Array(225);
  const oppHasPerpV = new Uint8Array(225);
  const oppCrossH = new Uint32Array(225);
  const oppCrossScoreH = new Int16Array(225);
  const oppHasPerpH = new Uint8Array(225);
  const oppAnchors = new Uint8Array(225);

  oppCrossV.fill(ALL_LETTERS_MASK);
  oppCrossH.fill(ALL_LETTERS_MASK);

  for (let r = 0; r < 15; r++) {
    for (let c = 0; c < 15; c++) {
      const gridIdx = r * 15 + c;
      if (testBoard[gridIdx] !== 0) {
      ...
      for (let k = 0; k < upCount; k++) {
        const code = testBoard[(r - upCount + k) * 15 + c] - 1;
        PERP_BUF[k] = code;
      ...
      while (down < 15 && testBoard[down * 15 + c] !== 0) {
        const code = testBoard[down * 15 + c] - 1;
      ...
      for (let k = 0; k < leftCount; k++) {
        const code = testBoard[r * 15 + (c - leftCount + k)] - 1;
        PERP_BUF[k] = code;
      ...
      while (right < 15 && testBoard[r * 15 + right] !== 0) {
        const code = testBoard[r * 15 + right] - 1;
  ...
  const testVector = (isVertical, lineIdx) => {
    ...
    for (let pos = 0; pos < 15; pos++) {
      if (lineA[pos] === 1) genOpp(pos, pos, 0, -1, pos, pos, 0);
    }
  };
  ...
  return { maxOppScore, bestOppWord };
}
...
self.onmessage = async function (e) {
  ...
  const searchVector = (isVertical, lineIdx) => {
    ...
    for (let pos = 0; pos < 15; pos++) {
      if (LINE_ANCHORS[pos] === 1) gen(pos, pos, 0, -1, pos, pos, 0);
    }
  };
  ...
  // Calculate Unseen Bag Composition based on user mode
  UNSEEN_COUNTS.fill(0);
  let totalUnseen = 0;

  if (enableIntel && manualAvailableTiles && manualAvailableTiles.trim()) {
    // If user pasted Woogles "Available Tiles", build pool exactly from the paste
    ...
    // Otherwise calculate dynamically from board + rack
    for (let i = 0; i < 26; i++) {
  ...
  // Determine Opponent Rack for Deduction/Minimax
  OPP_RACK_COUNTS.fill(0);
  let oppWildcards = 0;
  let isDeterministicOpponent = false;

  // If there are 7 or fewer tiles left in the Unseen Pool, we know exactly what the opponent has.
  ...
  // Exchange Valuation Logic (Uses the precise Unseen weights)
  ...
  for (let i = 0; i < resultsCount; i++) {
    ...
    for (let j = 0; j < finalPlays.length; j++) {
      ...
      if (
        ...
        let match = true;
        for (let k = 0; k < len; k++) {
          if (ex.word.charCodeAt(k) - 65 !== RES_WORD_CHARS[charOffset + k]) {
            match = false;
            break;
          }
        }
    ...
    // Run Minimax Counter-Move Search on Top Candidates
    if (isDeterministicOpponent && i < topCandidatesCount) {
      TEMP_BOARD_GRID.set(BOARD_GRID);
      for (let k = 0; k < len; k++) {
        const r = dir === "V" ? row + k : row;
        const c = dir === "H" ? col + k : col;
        TEMP_BOARD_GRID[r * 15 + c] = RES_WORD_CHARS[charOffset + k] + 1;
      }
  ...
  // Final Re-sort after Minimax Adjustments and Exchange Inclusion
  if (isDeterministicOpponent || bestExchange) {
    finalPlays.sort((a, b) => {
      if (sortMode === "score") {
