...
export default function ScrabbleSolverV3() {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      // Listen for Alt+O (using e.code to bypass macOS Option-key character mapping)
      ...
      if (/^[a-zA-Z]$/.test(e.key)) {
        ...
        // State Protection: Check if the cell is already occupied by the other player
        // This explicitly targets opponent tiles for blocking, mirroring the successful Backspace logic, while gracefully handling empty or orphan states.
        const opponentMode = inputModeRef.current === "me" ? "opp" : "me";
        if (existingTile && existingOwner === opponentMode) {
          // Smart Skip UX: If the user types the exact same letter that is already there,
          // just advance the cursor over the existing tile without mutating the board state or history.
          if (existingTile.toUpperCase() === typedChar.toUpperCase()) {
            if (typingDir === "Right" && c < 14) setSelectedCell([r, c + 1]);
            else if (typingDir === "Left" && c > 0) setSelectedCell([r, c - 1]);
            else if (typingDir === "Down" && r < 14) setSelectedCell([r + 1, c]);
            else if (typingDir === "Up" && r > 0) setSelectedCell([r - 1, c]);
          }
          // Block the overwrite completely
          return;
        }
        ...
        setBoard((prev) => {
          ...
          const char = e.shiftKey ? e.key.toLowerCase() : e.key.toUpperCase();
          next[r][c] = char;
          return next;
        });
        setTileOwners((prev) => {
        ...
        if (currentBoard[r][c]) {
          // State Protection: Explicitly block deletion if the tile is owned by the OPPOSITE of the current input mode.
          ...
          let prevR = r, prevC = c;
          if (typingDir === "Right" && c > 0) prevC = c - 1;
          else if (typingDir === "Left" && c < 14) prevC = c + 1;
          else if (typingDir === "Down" && r > 0) prevR = r - 1;
          else if (typingDir === "Up" && r < 14) prevR = r + 1;
          
          if (prevR !== r || prevC !== c) {
            const isPrevOpponentTile = currentOwner[prevR][prevC] === (inputModeRef.current === "me" ? "opp" : "me");
            if (isPrevOpponentTile) {
              setSelectedCell([prevR, prevC]);
              return;
            }
            setSelectedCell([prevR, prevC]);
            pushHistory();
            setBoard((prev) => { const next = prev.map(row => [...row]); next[prevR][prevC] = ""; return next; });
            setTileOwners((prev) => { const next = prev.map(row => [...row]); next[prevR][prevC] = ""; return next; });
          }
        }
  ...
  }, [isBoardLocked, typingDir, handleUndo, pushHistory]);
    ...
    <div className={`win98-body ${theme === "wood" ? "theme-hoyle" : ""}`}>
      ...
      <div className="win98-container">
        <div className="win98-window">
          ...
          <div className="win98-menubar"><span className="menu-item"><u>F</u>ile</span><span className="menu-item"><u>E</u>dit</span><span className="menu-item"><u>V</u>iew</span><span className="menu-item" style={{ cursor: "pointer" }} onClick={() => setShowTutorial(true)}><u>T</u>utorial</span>
          <span className="menu-item" style={{ cursor: "pointer" }} onClick={() => setShowHelp(true)}><u>H</u>elp</span></div>
  ...
  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "6px" }}>
    ...
    <select className="win98-input" style={{ width: "150px", cursor: "pointer", padding: "2px 4px" }} value={equityMode} onChange={(e) => setEquityMode(e.target.value)}>
      ...
      <option value="trained">Trained (ML)</option>
        ...
        {showTutorial && (
    <div className="win98-window" style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%, -50%)", zIndex: 10000, padding: "10px", width: "400px", maxWidth: "95vw", boxShadow: "2px 2px 10px rgba(0,0,0,0.5)" }}>
      <div className="win98-titlebar" style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px" }}>
  ...
  {showHelp && (
    <div className="win98-window" style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%, -50%)", zIndex: 10000, padding: "10px", width: "360px", boxShadow: "2px 2px 10px rgba(0,0,0,0.5)" }}>
      <div className="win98-titlebar" style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px" }}>
        <span>Help & Hotkeys</span>
        <button className="win98-button win98-btn-sys" onClick={() => setShowHelp(false)}>X</button>
      </div>
      <div className="win98-inset" style={{ padding: "10px", fontSize: "12px", lineHeight: "1.5", backgroundColor: "#fff" }}>
  ...
  {blankPrompt && (
    <div className="win98-window" style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%, -50%)", zIndex: 10000, padding: "10px", width: "300px", boxShadow: "2px 2px 10px rgba(0,0,0,0.5)" }}>
      <div className="win98-titlebar" style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px" }}>
